...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        array = []
        mapping = {None:'<ROOT>','CD':'<CD>','NNP':'<NNP>','UNK':'<UNK>'}
        for lst in [state.stack, state.buffer]:
            for i in range(1, 4):
                if i > len(lst):
                    array.append(self.word_vocab['<NULL>'])
                else:
                    temp = words[lst[-i]]
                    if temp in mapping:
                        array.append(self.word_vocab[mapping[temp]])
                    elif temp.lower() in self.word_vocab:
                        array.append(self.word_vocab[temp.lower()])
                    else:
                        array.append(self.word_vocab['<UNK>'])
            
        return np.array(array)
            
    def get_output_representation(self, output_pair):  
        out_repr = to_categorical(y=self.output_labels[output_pair], num_classes=91)
        return out_repr
